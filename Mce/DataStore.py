...
# GAMECHAIN START
    ...
    def get_gamechain_name_by_id(store, chain_id):
    ...
    def get_dirname_by_id(store, chain_id):
        """
        Get the chain name of a gamechain network, given the chain id.
        :param store:
        :param chain_id:
        :return:
        """
...
#    def _is_descended_from(store, block_id, ancestor_id):
    ...
    def _export_scriptPubKey(store, txout, chain, scriptPubKey):
        ...
        # GAMECHAIN START
        elif script_type == Chain.SCRIPT_TYPE_gamechain:
            txout['binaddr'] = data
        elif script_type == Chain.SCRIPT_TYPE_gamechain_P2SH:
            txout['address_version'] = chain.script_addr_vers
            txout['binaddr'] = data
        elif script_type == Chain.SCRIPT_TYPE_gamechain_ENTITY_PERMISSION:
    ...
    def import_tx(store, tx, is_coinbase, chain):
        ...
        for pos in xrange(len(tx['txOut'])):
            ...
            for row in store.selectall("""
                ...
                store.spent_txout_assets(tx['hash'], pos, chain, store.hashout(spender_hash), spender_n) # gamechain - deduct asset quantities now if couldn't find before
...
# GAMECHAIN START
            ...
            if pubkey_id is not None and script_type in [Chain.SCRIPT_TYPE_gamechain, Chain.SCRIPT_TYPE_gamechain_P2SH]:
                ...
                if data is not None:
                    ...
                    if opdrop_type==util.OP_DROP_TYPE_ISSUE_ASSET:
                        ...
                        if checksum is None:
                            ...
                            address = util.hash_to_address_gamechain(vers, pubkey_hash, checksum)
                        ...
                        pass
            elif pubkey_id is None and script_type is Chain.SCRIPT_TYPE_gamechain_OP_RETURN:
            ...
            elif pubkey_id is None and script_type is Chain.SCRIPT_TYPE_gamechain_SPKN:
                # Protocol 10007
                opdrop_type, val = util.parse_op_drop_data(data, chain)
...
# GAMECHAIN START
    def spent_txout_assets(store, prevout_hash, prevout_n, chain, spender_hash, spender_n):
        ...
        if binscript is not None:
            ...
            if the_script_type is Chain.SCRIPT_TYPE_gamechain_ENTITY_PERMISSION:
                pubkey_hash = data['pubkey_hash']
            else:
                pubkey_hash = data

            if the_script_type is Chain.SCRIPT_TYPE_gamechain_P2SH:
            ...
            if the_script_type in [Chain.SCRIPT_TYPE_gamechain, Chain.SCRIPT_TYPE_gamechain_P2SH]:
                data = util.get_gamechain_op_drop_data(binscript)
                if data is not None:
                    opdrop_type, val = util.parse_op_drop_data(data, chain)
                    if opdrop_type==util.OP_DROP_TYPE_ISSUE_ASSET:
...
# GAMECHAIN START
            ret['gamechain_scriptPubKey'] = scriptPubKey if len(row) > 5 else None
# GAMECHAIN END
            return ret
...
# GAMECHAIN START
        if script_type == Chain.SCRIPT_TYPE_gamechain_P2SH:
            return store.pubkey_hash_to_id(data, PUBKEY_FLAGS_P2SH)

        if script_type == Chain.SCRIPT_TYPE_gamechain:
            return store.pubkey_hash_to_id(data)

        if script_type == Chain.SCRIPT_TYPE_gamechain_ENTITY_PERMISSION:
...
# GAMECHAIN START
        # Expand ~ in gamechain folder to user home directory
        dircfg['dirname'] = os.path.expanduser(dircfg['dirname'])
        # Extract chain name from last path component of gamechain folder
        ...
        def get_blockhash(height):
            try:
                ...
                if e.code in (-1, -5, -8, -711):
                    ...
                    #-711 (RPC_BLOCK_NOT_FOUND): gamechain block not found error
                    return None
                raise
...
# GAMECHAIN START
    ...
    def get_assets(store, chain):
        ...
        gamechain_name = store.get_gamechain_name_by_id(chain.id)
        resp = None
        try:
    ...
    def get_asset_by_name(store, chain, name):
        ...
        resp = None
        try:
            resp = util.jsonrpc(gamechain_name, url, "listassets", name)
            if len(resp) > 0 :
                resp = resp[0]
        except util.JsonrpcException as e:
            raise Exception("JSON-RPC error({0}): {1}".format(e.code, e.message))
    ...
    def get_block_by_hash(store, chain, blockhash):
        ...
        resp = None
        try:
            resp = util.jsonrpc(gamechain_name, url, "getblock", blockhash)
        except util.JsonrpcException as e:
            raise Exception("JSON-RPC error({0}): {1}".format(e.code, e.message))
        except IOError as e:
            raise e
    ...
    def get_peerinfo(store, chain):
        ...
        resp = None
        try:
            resp = util.jsonrpc(gamechain_name, url, "getpeerinfo")
        except util.JsonrpcException as e:
            raise Exception("JSON-RPC error({0}): {1}".format(e.code, e.message))
        except IOError as e:
            raise e
...
#                "outpos": int(pos),
    ...
    def list_transactions(store, chain, count):
        ...
        NOTE: This call does not work with gamechain v2 scalable wallets.
        :param chain:
        :return: json object
        """
        url = store.get_url_by_chain(chain)
        ...
        resp = None
        try:
            resp = util.jsonrpc(gamechain_name, url, "listtransactions", "*", count)
        except util.JsonrpcException as e:
            raise Exception("JSON-RPC error({0}): {1}".format(e.code, e.message))
        except IOError as e:
            raise e
    ...
    def list_stream(store, chain, name):
        ...
        resp = None
        try:
            resp = util.jsonrpc(gamechain_name, url, "liststreams", name, True)
        except util.JsonrpcException as e:
            raise Exception("JSON-RPC error({0}): {1}".format(e.code, e.message))
        except IOError as e:
            raise e
    ...
    def list_streams(store, chain, count=0):
        ...
        try:
            ...
            # all streams
            if count <= 0:
                resp = util.jsonrpc(gamechain_name, url, "liststreams", "*", True)
            else:
                resp = util.jsonrpc(gamechain_name, url, "liststreams", "*", True, count, -count)
        except util.JsonrpcException as e:
            raise Exception("JSON-RPC error({0}): {1}".format(e.code, e.message))
        except IOError as e:
            raise e
    ...
    def list_stream_items(store, chain, streamref, count=10, start=-10):
        ...
        try:
            ...
            resp = util.jsonrpc(gamechain_name, url, "liststreamitems", streamref, True, count, start)
        except util.JsonrpcException as e:
            raise Exception("JSON-RPC error({0}): {1}".format(e.code, e.message))
        except IOError as e:
            raise e
    ...
    def list_stream_publisher_items(store, chain, streamref, publisher, count=10, start=-10):
        ...
        try:
            ...
            resp = util.jsonrpc(gamechain_name, url, "liststreampublisheritems", streamref, publisher, True, count, start)
        except util.JsonrpcException as e:
            raise Exception("JSON-RPC error({0}): {1}".format(e.code, e.message))
        except IOError as e:
            raise e
    ...
    def list_stream_publishers(store, chain, streamref, publisher="*"):
        ...
        try:
            ...
            resp = util.jsonrpc(gamechain_name, url, "liststreampublishers", streamref, publisher)
        except util.JsonrpcException as e:
            raise Exception("JSON-RPC error({0}): {1}".format(e.code, e.message))
        except IOError as e:
            raise e
    ...
    def list_stream_keys(store, chain, streamref, keys="*"):
        ...
        try:
            ...
            resp = util.jsonrpc(gamechain_name, url, "liststreamkeys", streamref, keys)
        except util.JsonrpcException as e:
            raise Exception("JSON-RPC error({0}): {1}".format(e.code, e.message))
        except IOError as e:
            raise e
    ...
    def list_stream_key_items(store, chain, streamref, streamkey, count=10, start=-10):
        ...
        try:
            ...
            resp = util.jsonrpc(gamechain_name, url, "liststreamkeyitems", streamref, streamkey, True, count, start)
        except util.JsonrpcException as e:
            raise Exception("JSON-RPC error({0}): {1}".format(e.code, e.message))
        except IOError as e:
            raise e
    ...
    def get_rawmempool(store, chain):
        ...
        resp = None
        try:
            resp = util.jsonrpc(gamechain_name, url, "getrawmempool", True)
        except util.JsonrpcException as e:
            raise Exception("JSON-RPC error({0}): {1}".format(e.code, e.message))
        except IOError as e:
            raise e
    ...
    def get_rawtransaction_decoded(store, chain, tx_hash):
        ...
        resp = None
        try:
            resp = util.jsonrpc(gamechain_name, url, "getrawtransaction", tx_hash, 1)
        except util.JsonrpcException as e:
            raise Exception("JSON-RPC error({0}): {1}".format(e.code, e.message))
        except IOError as e:
            raise e
    ...
    def get_labels_for_scriptpubkey(store, chain, scriptpubkey):
        ...
        :param chain:
        :param scriptpubkey:
        :return: labels as list, or empty list if not a recognized gamechain OP_DROP or OP_RETURN.
        """
        label = []
        script_type, data = chain.parse_txout_script(scriptpubkey)
        if script_type is Chain.SCRIPT_TYPE_gamechain_P2SH:
            label.append('P2SH')
        if script_type in [Chain.SCRIPT_TYPE_gamechain_STREAM,
            Chain.SCRIPT_TYPE_gamechain_STREAM_ITEM]:
            label.append('Stream')
        if script_type in [Chain.SCRIPT_TYPE_gamechain, Chain.SCRIPT_TYPE_gamechain_P2SH]:
            data = util.get_gamechain_op_drop_data(scriptpubkey)
            if data is not None:
                opdrop_type, val = util.parse_op_drop_data(data, chain)
                if opdrop_type==util.OP_DROP_TYPE_ISSUE_ASSET:
                    label.append('Asset')
