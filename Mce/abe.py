...
# GAMECHAIN START
ABE_APPNAME = "GameChain Explorer"
ABE_VERSION = __version__
ABE_URL = 'https://github.com/tedchain/gamechain-explorer'

COPYRIGHT_YEARS = '2011-2017'
COPYRIGHT = "Coin Sciences Ltd and Abe developers"
COPYRIGHT_URL = 'https://github.com/tedchain/gamechain-explorer'
# GAMECHAIN END
...
<body>
    <div class="container">
	<table><td>
	<a title="Back to home" href="%(dotdot)s%(HOMEPAGE)s"><img src="%(dotdot)s%(STATIC_PATH)slogo32.png" alt="gamechain logo" /></a>
...
# GAMECHAIN START
    ...
    def get_blockchainparams(abe, chain):
        ...
        gamechain_name = abe.store.get_gamechain_name_by_id(chain.id)
        params = {}
        try:
            params = util.jsonrpc(gamechain_name, url, "getblockchainparams")
...
# GAMECHAIN START
                ...
                body += '<td>'
                if chain.__class__.__name__ is "gamechain":
                    if num_assets == -1:
                        body += '?'
                    elif num_assets>=0:
...
# GAMECHAIN_START
    def handle_chain(abe, page):
        ...
        params_resp = None
        try:
            info_resp = util.jsonrpc(gamechain_name, url, "getinfo")
            params_resp = util.jsonrpc(gamechain_name, url, "getblockchainparams")
...
# GAMECHAIN START
            # Describe gamechain specific transaction
            labels = []
            labeltype = 'success'
            try:
            ...
            if mytx is not None:
                for txout in mytx['out']:
                    ...
                    if script_type in [Chain.SCRIPT_TYPE_GAMECHAIN, Chain.SCRIPT_TYPE_GAMECHAIN_P2SH]:
                        data = util.get_gamechain_op_drop_data(txout['binscript'])
                        if data is not None:
                            opdrop_type, val = util.parse_op_drop_data(data, chain)
                            label = util.get_op_drop_type_description(opdrop_type)
                        else:
                            label = 'Unknown gamechain command'
                            labeltype = 'danger'

                    elif script_type is Chain.SCRIPT_TYPE_GAMECHAIN_OP_RETURN:
                        opreturn_type, val = util.parse_op_return_data(data, chain)
                        label = util.get_op_return_type_description(opreturn_type)

                    elif script_type is Chain.SCRIPT_TYPE_GAMECHAIN_STREAM:
                        label = 'Create Stream'

                    elif script_type is Chain.SCRIPT_TYPE_GAMECHAIN_STREAM_ITEM:
                        label = 'Stream Item'

                    elif script_type is Chain.SCRIPT_TYPE_GAMECHAIN_ENTITY_PERMISSION:
                        label = 'Entity Permission'

                    elif script_type in [Chain.SCRIPT_TYPE_GAMECHAIN_SPKN, Chain.SCRIPT_TYPE_GAMECHAIN_SPKU]:
                        data = util.get_gamechain_op_drop_data(txout['binscript'])
                        if data is not None:
                            opdrop_type, val = util.parse_op_drop_data(data, chain)
                            label = util.get_op_drop_type_description(opdrop_type)
...
# GAMECHAIN START
    ...
    def show_tx_row_to_html_impl(abe, chain, body, asset_txid_dict, binscript, script_type, data):
        ...
        msgtype = 'success'
        msgpanelstyle = ''
        if script_type in [Chain.SCRIPT_TYPE_GAMECHAIN,
                           Chain.SCRIPT_TYPE_GAMECHAIN_P2SH,
                           Chain.SCRIPT_TYPE_GAMECHAIN_STREAM,
                           Chain.SCRIPT_TYPE_GAMECHAIN_STREAM_ITEM,
                           Chain.SCRIPT_TYPE_GAMECHAIN_SPKN,  # also matches template used for input cache opdrop
                           Chain.SCRIPT_TYPE_GAMECHAIN_SPKU]:
            # NOTE: data returned above is pubkeyhash, due to common use to get address, so we extract data ourselves.
            data = util.get_gamechain_op_drop_data(binscript)
            if data is not None:
                opdrop_type, val = util.parse_op_drop_data(data, chain)
                if opdrop_type==util.OP_DROP_TYPE_ISSUE_ASSET:
                    ...
                    msg = 'Create stream:'
                    if chain.protocol_version < 10007:
                        data = util.get_gamechain_op_return_data(binscript)
                        opreturn_type, val = util.parse_op_return_data(data, chain)
                        fields = val['fields']
                    else:
                ...
                elif opdrop_type==util.OP_DROP_TYPE_SPKE and script_type==Chain.SCRIPT_TYPE_GAMECHAIN_SPKU:
                ...
                elif (opdrop_type==util.OP_DROP_TYPE_SPKE and script_type==Chain.SCRIPT_TYPE_GAMECHAIN_STREAM_ITEM) or opdrop_type == util.OP_DROP_TYPE_STREAM_ITEM:
        ...
        if script_type is Chain.SCRIPT_TYPE_GAMECHAIN_ENTITY_PERMISSION:
        ...
        if script_type is Chain.SCRIPT_TYPE_GAMECHAIN_OP_RETURN:
            opreturn_type, val = util.parse_op_return_data(data, chain)
    ...
    def show_tx(abe, page, tx):
        ...
        def row_to_html(row, this_ch, other_ch, no_link_text):
            ...
            if binscript is not None:
                ...
                if script_type in [Chain.SCRIPT_TYPE_GAMECHAIN_OP_RETURN,
                                   Chain.SCRIPT_TYPE_GAMECHAIN_STREAM,
                                   Chain.SCRIPT_TYPE_GAMECHAIN_STREAM_ITEM,
                                   Chain.SCRIPT_TYPE_GAMECHAIN_SPKN,
                                   Chain.SCRIPT_TYPE_GAMECHAIN_SPKU]:
                    novalidaddress = True
...
# GAMECHAIN START
        ...
        body += [' <a role="button" class="btn btn-default btn-xs" href="../rpctxjson/', tx['hash'], '">GameChain JSON</a>']
        body += [' <a role="button" class="btn btn-default btn-xs" href="../rpctxraw/', tx['hash'], '">GameChain Hex</a>']
        body += ['</p>']
# GAMECHAIN END
    ...
    def show_mempool_tx_json(abe, page, tx):
        ...
        def row_to_html(row, this_ch, other_ch, no_link_text):
            ...
            if addressLabel is not 'None':
                version, pubkeyhash = util.decode_address_gamechain(addressLabel)
        ...
        chain_name = abe.store.get_gamechain_name_by_id(chain.id)
...
# GAMECHAIN START
    ...
    def handle_address(abe, page):
        ...
        # check the address
        version, pubkeyhash = util.decode_check_address_gamechain(address)
        if pubkeyhash is None:
            raise PageNotFound()
            #raise MalformedAddress("Invalid address")

        # If the HTML link for this handler gets only created for gamechain networks, we don't need to check class.
        #if chain.__class__.__name__ is "gamechain":
        body += ['<h3>Permissions</h3>']
        try:
            resp = util.jsonrpc(gamechain_name, url, "listpermissions", "all", address)
            if len(resp) > 0 :
                body += ['<ul>']
        ...
        if abe.get_blockchainparams(chain).get('initial-block-reward', 0) > 0:
            ...
            try:
                resp = util.jsonrpc(gamechain_name, url, "getaddressbalances", address)
                if len(resp) is 0:
                    body += ['None']
                else:
                    body += ['<ul>']
    ...
    def handle_assetaddress(abe, page):
        ...
        try:
            resp = util.jsonrpc(gamechain_name, url, "listassets", assetref)
            asset = resp[0]
        except util.JsonrpcException as e:
            msg= "JSON-RPC error({0}): {1}".format(e.code, e.message)
        ...
        def get_asset_amount_from_txinout(tx, this_ch, other_ch, asset, chain):
            ...
            if checksum is None:
                ...
                addr = util.hash_to_address_gamechain(vers, binaddr, checksum)
            ...
            if this_ch=='i':
                binscript = tx['gamechain_scriptPubKey']

            script_type, data = chain.parse_txout_script(binscript)

            if script_type not in [Chain.SCRIPT_TYPE_GAMECHAIN, Chain.SCRIPT_TYPE_GAMECHAIN_P2SH]:
                return 0
    ...
    def handle_assetref(abe, page):
        ...
        # get asset information and issue tx as json
        try:
            resp = util.jsonrpc(gamechain_name, url, "listassets", assetref, True) # verbose to get 'issues' field
            asset = resp[0]
            issuetxid = asset['issuetxid']
            resp = util.jsonrpc(gamechain_name, url, "getrawtransaction", issuetxid, 1)
            issuetx = resp
        except util.JsonrpcException as e:
            msg= "JSON-RPC error({0}): {1}".format(e.code, e.message)
            body += ['<div class="alert alert-danger" role="warning">', msg ,'</div>']
...
#                                                                                                                '' b['height'] if b['height'] is not None else '')
        ...
        body += ['<p class="text-right">']
        body += ['<button href="#RawJson" class="btn btn-default btn-xs" data-toggle="collapse">GameChain JSON</button>']
...
# GAMECHAIN START
            ...
            if checksum is None:
                ...
                address = util.hash_to_address_gamechain(vers, pubkeyhash, checksum)
...
# GAMECHAIN END
    ...
    def handle_assets(abe, page):
        ...
        gamechain_name = abe.store.get_gamechain_name_by_id(chain.id)
        try:
            resp = util.jsonrpc(gamechain_name, url, "listassets")
            num_assets = len(resp)
        except util.JsonrpcException as e:
            msg= "JSON-RPC error({0}): {1}".format(e.code, e.message)
            body += ['<div class="alert alert-danger" role="warning">', msg ,'</div>']
...
# GAMECHAIN START
                    ...
                    if checksum is None:
                        ...
                        other = util.hash_to_address_gamechain(vers, binaddr, checksum)
# GAMECHAIN END
                    if other != address:
...
# GAMECHAIN START
            ...
            chain = abe.store.get_chain_by_id(1)
            version, binaddr = util.decode_check_address_gamechain(address)
# GAMECHAIN END
        except Exception:
...
# GAMECHAIN START
    def search_address_prefix(abe, ap):
        ...
        def process(row):
            ...
            if hash is None:
                return None
            address = util.hash_to_address_gamechain(address_version, hash, checksum)
...
# GAMECHAIN START
        version, hash = util.decode_check_address_gamechain(addr)
# GAMECHAIN END
        if hash is None:
...
# GAMECHAIN START
    if checksum is None:
        ...
        addr = util.hash_to_address_gamechain(version, hash, checksum)
# GAMECHAIN END
    if text is not None:
        visible = text
