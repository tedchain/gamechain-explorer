...
# GAMECHAIN START
def hash_to_address_gamechain(version, hash, checksum):
    """
    Format address the gamechain way, with version bytes and checksum bytes.
    http://www.gamechain.com/developers/address-format/
...
def decode_check_address_gamechain(address):
    version, hash = decode_address_gamechain(address)
    if version is not None and hash is not None:
        raw = base58.b58decode(address, None)
        checksum = raw[-4:0]
        if hash_to_address_gamechain(version, hash, checksum):
            return version, hash
    return None, None

def decode_address_gamechain(address):
...
# GAMECHAIN START
# gamechain requires chain_name argument in JSON requests.
...
def parse_create_stream_10007(data):
    ...
    while pos<len(data):
        # Is this a special property with meaning only for gamechain?
        if data[pos:pos+1] == "\0":
...
def parse_follow_on_issuance_metadata_10007(data):
    ...
    while pos<len(data):
        ...
        if data[pos:pos+1] == "\0":
            ...
            assetprop = data[pos+3:pos+3+assetproplen]
            proptype = ord(data[pos+1])
            fname = "gamechain special property at offset {0}".format(pos)
            fields[fname] = long_hex(assetprop)
            pos = pos + 3 + assetproplen
            continue
...
def get_gamechain_op_drop_data(script):
    ...
    if deserialize.match_decoded(decoded, Chain.SCRIPT_GAMECHAIN_TEMPLATE):
        data = decoded[5][1]
    elif deserialize.match_decoded(decoded, Chain.SCRIPT_GAMECHAIN_P2SH_TEMPLATE):
        data = decoded[3][1] # 4th element contains the OP_DROP data.
    elif deserialize.match_decoded(decoded, Chain.SCRIPT_GAMECHAIN_STREAM_ITEM_TEMPLATE):
        data = decoded[0][1] # 1st element contains the creation txid OP_DROP data
    elif deserialize.match_decoded(decoded, Chain.SCRIPT_GAMECHAIN_STREAM_TEMPLATE):
        data = decoded[0][1] # 1st element contains the OP_DROP data.
    elif deserialize.match_decoded(decoded, Chain.SCRIPT_GAMECHAIN_SPKN_TEMPLATE):
        data = decoded[0][1] # 1st element contains the OP_DROP data.
    elif deserialize.match_decoded(decoded, Chain.SCRIPT_GAMECHAIN_FOLLOW_ON_ISSUANCE_METADATA_TEMPLATE):
        data = decoded[0][1] # 1st element contains the OP_DROP data.
    return data
...
def get_gamechain_op_return_data(script):
    ...
    try:
        ...
        return None
    data = None
    if deserialize.match_decoded(decoded, Chain.SCRIPT_GAMECHAIN_STREAM_TEMPLATE):
