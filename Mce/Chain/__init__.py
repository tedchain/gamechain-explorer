...
# GAMECHAIN START
# Template to match a gamechain permission or asset transaction which puts a payload before OP_DROP
SCRIPT_gamechain_TEMPLATE = [
...
# Template to match a gamechain entity permission command
SCRIPT_gamechain_ENTITY_PERMISSION_TEMPLATE = [
...
# Template to match a gamechain stream item
# spke for stream item
SCRIPT_gamechain_STREAM_ITEM_TEMPLATE = [ opcodes.OP_PUSHDATA4, opcodes.OP_DROP, opcodes.OP_PUSHDATA4, opcodes.OP_DROP, opcodes.OP_RETURN, opcodes.OP_PUSHDATA4 ]

# spke for asset
SCRIPT_gamechain_FOLLOW_ON_ISSUANCE_METADATA_TEMPLATE = [ opcodes.OP_PUSHDATA4, opcodes.OP_DROP, opcodes.OP_PUSHDATA4, opcodes.OP_DROP, opcodes.OP_RETURN]

# Template to match a gamechain create stream command
SCRIPT_gamechain_STREAM_TEMPLATE = [ opcodes.OP_PUSHDATA4, opcodes.OP_DROP, opcodes.OP_RETURN, opcodes.OP_PUSHDATA4 ]

# TODO: Rename as the template also matches spki for input cache
# spkn, for both new issue and create stream
SCRIPT_gamechain_SPKN_TEMPLATE = [opcodes.OP_PUSHDATA4, opcodes.OP_DROP, opcodes.OP_RETURN ]

# Matches all opcodes < PUSHDATA4
# See: https://en.bitcoin.it/wiki/Script
SCRIPT_gamechain_OP_RETURN_TEMPLATE = [ opcodes.OP_RETURN, opcodes.OP_PUSHDATA4 ]

# Template to match a gamechain asset pay-to-script-hash (P2SH) output script
# e.g. "OP_HASH160 f1191c44953b7da866dba982d155bbd8eeb89dfc OP_EQUAL 73706b7104000000f70200001ae41027000000000000 OP_DROP"
SCRIPT_gamechain_P2SH_TEMPLATE = [ opcodes.OP_HASH160, PUBKEY_HASH_LENGTH, opcodes.OP_EQUAL, opcodes.OP_PUSHDATA4, opcodes.OP_DROP]
...
SCRIPT_TYPE_P2SH = 6
# GAMECHAIN START
SCRIPT_TYPE_gamechain = 7
SCRIPT_TYPE_gamechain_OP_RETURN = 8
SCRIPT_TYPE_gamechain_P2SH = 9
SCRIPT_TYPE_gamechain_STREAM = 10
SCRIPT_TYPE_gamechain_STREAM_ITEM = 11
SCRIPT_TYPE_gamechain_ENTITY_PERMISSION = 12
SCRIPT_TYPE_gamechain_SPKN = 13
SCRIPT_TYPE_gamechain_SPKU = 14    # follow on asset issuance metadata
# GAMECHAIN END
...
# GAMECHAIN START
        * SCRIPT_TYPE_gamechain - DATA is the binary public key (there is another method to get the OPDROP data)
        * SCRIPT_TYPE_gamechain_P2SH - DATA is the binary script hash (there is another method to get the OPDROP data)
        * SCRIPT_TYPE_gamechain_STREAM - DATA is a dicationary containing op_drop and op_return data.
        * SCRIPT_TYPE_gamechain_STREAM_ITEM - Data is a dictionary containing stream creation txid, item key, item data.
        * SCRIPT_TYPE_gamechain_STREAM_PERMISSION - Data is a dictionary containing stream creation txid, permissions, pubkey_hash
        * SCRIPT_TYPE_gamechain_SPKN
        * SCRIPT_TYPE gamechain_SPKE
# GAMECHAIN END
        """
        if script is None:
            raise ValueError()
...
# GAMECHAIN START
        # Return dict
        if deserialize.match_decoded(decoded, SCRIPT_gamechain_ENTITY_PERMISSION_TEMPLATE):
            dict = {"txid":decoded[5][1], "permissions":decoded[7][1], "pubkey_hash":decoded[2][1]}
            return SCRIPT_TYPE_gamechain_ENTITY_PERMISSION, dict

        # Return script type and address
        if deserialize.match_decoded(decoded, SCRIPT_gamechain_TEMPLATE):
            pubkey_hash = decoded[2][1]
            if len(pubkey_hash) == PUBKEY_HASH_LENGTH:
                return SCRIPT_TYPE_gamechain, pubkey_hash

        # Return dict
        if deserialize.match_decoded(decoded, SCRIPT_gamechain_STREAM_ITEM_TEMPLATE):
            dict = {"streamtxid":decoded[0][1], "itemkey":decoded[2][1], "itemdata":decoded[5][1]}
            return SCRIPT_TYPE_gamechain_STREAM_ITEM, dict

        # Return dict
        if deserialize.match_decoded(decoded, SCRIPT_gamechain_STREAM_TEMPLATE):
            dict = {"op_drop":decoded[0][1], "op_return":decoded[3][1]}
            return SCRIPT_TYPE_gamechain_STREAM, dict

        if deserialize.match_decoded(decoded, SCRIPT_gamechain_FOLLOW_ON_ISSUANCE_METADATA_TEMPLATE):
            dict = {"assetidentifier":decoded[0][1], "assetdetails":decoded[2][1]}
            return SCRIPT_TYPE_gamechain_SPKU, dict

        #SCRIPT_gamechain_SPKN_TEMPLATE
        if deserialize.match_decoded(decoded, SCRIPT_gamechain_SPKN_TEMPLATE):
            metadata = decoded[0][1]
            return SCRIPT_TYPE_gamechain_SPKN, metadata

        # Return script type and metadata byte array
        elif deserialize.match_decoded(decoded, SCRIPT_gamechain_OP_RETURN_TEMPLATE):
            metadata = decoded[1][1]
            return SCRIPT_TYPE_gamechain_OP_RETURN, metadata

        # Return script type and script hash
        elif deserialize.match_decoded(decoded, SCRIPT_gamechain_P2SH_TEMPLATE):
            script_hash = decoded[1][1]
            assert len(script_hash) == PUBKEY_HASH_LENGTH
            return SCRIPT_TYPE_gamechain_P2SH, script_hash
